...
const NoneIndexType None = NoneIndexType();
const EllipsisIndexType Ellipsis = EllipsisIndexType();

SliceIndex::SliceIndex(at::indexing::NoneIndexType none) : type_(SliceIndexType::None) {}
SliceIndex::SliceIndex(int64_t integer) : integer_(integer), type_(SliceIndexType::Integer) {}

bool SliceIndex::is_none() const {
  return type_ == SliceIndexType::None;
}

bool SliceIndex::is_integer() const {
  return type_ == SliceIndexType::Integer;
}

int64_t SliceIndex::integer() const {
  return integer_;
}
...
TensorIndex::TensorIndex(at::indexing::NoneIndexType) : type_(TensorIndexType::None) {}
...
TensorIndex::TensorIndex(const char *str) : TensorIndex(at::indexing::Ellipsis) {
  TORCH_CHECK(strcmp(str, "...") == 0, "Expected \"...\" to represent an ellipsis index, but got \"", str, "\"");
}
...
TensorIndex::TensorIndex(std::initializer_list<SliceIndex> init_list) : type_(TensorIndexType::Slice) {
  if (init_list.size() == 0) {
    slice_ = unpackSlice();
    ...
    TORCH_CHECK(false, "Expected 0 / 2 / 3 elements in the braced-init-list to represent a slice, but got ", init_list.size(), " element(s)");
  }
}
...
// int64_t count_specified_dimensions(const std::vector<TensorIndex>& indices) {
...
// Tensor applySlicing(const Tensor& self, const std::vector<TensorIndex>& indices, std::vector<Tensor>& outIndices) {
...
// Tensor get_item(const Tensor& self, const std::vector<TensorIndex>& indices) {
//   OptionalDeviceGuard device_guard(device_of(self));
...
// void set_item(Tensor& self, const std::vector<TensorIndex>& indices, const Tensor& value) {
//   OptionalDeviceGuard device_guard(device_of(self));
...
// void set_item(Tensor& self, const std::vector<TensorIndex>& indices, Scalar v) {
//   OptionalDeviceGuard device_guard(device_of(self));
//   Tensor value;
